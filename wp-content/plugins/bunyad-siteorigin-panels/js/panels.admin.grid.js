...
(function($){
    ...
    panels.createGrid = function ( cells, weights, style ) {
        ...
        for (var index in weights) {
        	...
        	if (!weights.hasOwnProperty(index)) {
        		continue;
        	}
    ...
    panels.setupGrid = function ( $$ ) {
        ...
        $$.find( '.grid .cell' ).not( '.first' ).each( function () {
            ...
            var columns =  $$.find( '.grid .cell' ).length;
            ...
            /*$( this ).resizable( {
                ...
                stop:       function ( event, ui ) {
                    ...
                    //$(column).resizable("option","maxWidth",parseInt($(column).width(), 10) + 200);
                    //$(column).resizable("option","minWidth",parseInt($(column).width(), 10) - 20);
                },
                grid: grid,
        ...
        $$.find( '.grid .cell' )
            ...
            .sortable( {
                ...
                items: ".panel:not(.ui-state-disabled)",
                tolerance:  'pointer',
                ...
                change: function (ui) {
                    ...
                    // hide placeholder for special block containers
                    if ($('#panels-container .cell-selected').closest('.grid-container').hasClass('no-container')) {
                    	
                    	$('.ui-state-highlight').css('display', 'none');
                    	
                    	// resize - will get messy with moving
