...
jQuery( function ( $ ) {
    ...
    gridAddDialogButtons[panels.i10n.buttons.add] = function () {
        var num = $('#grid-add-dialog').find('select').val();
        
        if (!num) {
        	num =  $('#grid-add-dialog').find('[name=column_count]').val();
        }

        var cols = [], total = 0, stop = 0;
        $.each(num.split('+'), function(k, v) {

        	// split 1/2 and divide to get weight
        	v = v.split('/');
        	v = parseInt(v[0]) / parseInt(v[1]);

        	if (isNaN(v)) {
        		alert(panels.i10n.messages.invalid_row_format);
        		stop = 1;
        		return false;
        	}
        ...
        if (total < 0.999) {
            alert(panels.i10n.messages.invalid_column_count.replace('{total}', total*100));
            return false;
        }

        // Make sure the number is between 1 and 10.
        //num = Math.min( 10, Math.max( 1, Math.round( num ) ) );
        var gridContainer = window.panels.createGrid( cols.length, cols );
    ...
    // show add row custom column count when selected 
    $('#grid-add-dialog').on('change', 'select', function() {
    	
    	var ele = $('#grid-add-dialog').find('[name=column_count]').closest('p');
    ...
    $( '#grid-add-dialog' )
        ...
        .dialog( {
            ...
            open: function() {
            	$('#grid-add-dialog').find('select').trigger('change');
            },
        ...
        .prepend(
            $( '<a id="content-panels" class="hide-if-no-js wp-switch-editor switch-panels">' + $( '#so-panels-panels h3.hndle span' ).html() + '</a>' )
                .click( function () {
                    ...
                    // activate our page template
                    $('#page_template').val('page-blocks.php');
